...
/**
 * Trava de frequência — não é sobre requisição concorrente (o `carregamento`
 * acima já resolve isso), é sobre CHAMADA REPETIDA no tempo. O efeito que
 * dispara `carregar()` roda em todo re-render de quem usa `useSessao()` — de
 * propósito, é como ele nota renovação de token feita por outra chamada. Mas
 * se esse cheque ficar preso divergindo por qualquer motivo (um bug em outro
 * canto, uma condição de corrida nunca prevista), sem isto ele bateria em
 * `/auth/eu` a cada render, sem freio — foi assim que uma tela aberta gerou
 * milhares de chamadas e derrubou o rate limit da API numa sessão real.
 */
...
function carregar() {
  ...
  if (agora - ultimaTentativa < INTERVALO_MINIMO_MS) return Promise.resolve();
