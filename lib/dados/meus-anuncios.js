...
const FORMATO_DATA_HORA = new Intl.DateTimeFormat('pt-BR', { dateStyle: 'medium', timeStyle: 'short' });

/* a API manda ISO cru (`2026-08-17T00:05:03.593Z`) — sem formatar aqui, a
   tela mostrava esse texto direto, que ninguém lê como data */
function quando(valor) {
  return valor ? FORMATO_DATA_HORA.format(new Date(valor)) : '—';
}
...
/**
 * O que o `LocationMap` espera — mesma função de `lib/dados/anuncio.js`
 * (vitrine pública), duplicada porque as duas telas leem formas diferentes
 * do mesmo `GET /anuncios/:id` (aqui é a versão "dono", lá é a pública) e não
 * têm por que compartilhar módulo só por causa de oito linhas.
 */
function paraLocal(item) {
  const municipioNome = item.localizacao?.municipio?.nome || '';
  const uf = item.localizacao?.uf || '';
  const coordenada = item.localizacao?.coordenada;

  if (!municipioNome && !coordenada) return null;
